...
class HRVacationMapEdit extends Component {
	constructor(props) {
		super(props);
	...
	handleSelectedYear(e) {
		...
		if (e.target.value === '0') {
			...
			$('.row:last-of-type .hrvmedit_content').css({'opacity': '0.5', 'pointer-events': 'none'});
			// Make already grayed-out employees fully visible, so that they don't get double-grayed-out
			$('#hrvmedit_col_left .list-group-item').css({'opacity': '1', 'pointer-events': 'none'});
			// Unselect currently selected employee (if any)
			$('#hrvmedit_col_left .list-group-item').removeClass('list-group-item-active');
		// Otherwise, if an actual year was selected
		} else {
			// Make whole page - except year selection form field - active
			$('.row:last-of-type .hrvmedit_content').css({'opacity': '1', 'pointer-events': 'auto'});
			...
			this.state.employeeListData.map((item, index) => {
				...
				// ...make that employee active, so that we can edit/change the v0 details
				if (yearList.includes(Number(e.target.value))) {
					$('#hrvmedit_col_left .list-group-item:nth-of-type(' + (index + 1) + ')').css({'opacity': '1', 'pointer-events': 'auto'});
					employeeNameList.push(item.name);
				// Current employee doesn't have a VM for the selected year, so make that employee inactive, we need to create a v0 first
				// Also add that employee to the autosuggest list. This way, we can only search for employees that already have a v0 ;)
				} else {
					$('#hrvmedit_col_left .list-group-item:nth-of-type(' + (index + 1) + ')').css({'opacity': '0.5', 'pointer-events': 'none'});
				}
	...
	processAutosuggest(newValue) {
		...
		this.state.employeeListData.map(item => {
			if (item.name === newValue) {
				...
				item.vacation_maps.map(item2 => {
					if (item2.year.toString() === this.state.selectedYear) {
						this.setState({ 
							allowedVacationDays: item2.allowed_days,
							allowedOtherVacationDays: item2.other_days,
						});
	...
	handleSubmit(e) {
		...
		if (!isFormValid) {
			...
			// Update vacation_days and other_days
			api.vacation_maps.editAllowedDays(this.state.selectedYear, this.state.selectedEmployeeId, this.state.allowedVacationDays, this.state.allowedOtherVacationDays)
			.then((response) => {
				if (response.ok)
			...
			// Do general page maintenance (clear states, un-highlight employee in question, display submit message)
			this.setState({
				selectedEmployeeId: '',
				selectedEmployeeName: '',
				allowedVacationDays: '',
			...
			$("#submitSuccessful").show().text('Vacation map updated successfully.').delay(3000).fadeOut();
		}
	}
	...
	handleSelectedEmployee(e, selectedEmployeeId, selectedEmployeeName) {
		...
		// Get the selected employee allowed_days and other_days
		this.state.employeeListData.map((item, index) => {
	...
	render() {
		...
		$(document).ready(function() {
			const $listGroup = $('#hrvmedit_col_left .list-group-item');
					...
					<div className="container-fluid">
						...
						<div className="row">
							<div className="col-12">
								<div className="hrvmedit_content">
						...
						<div className="row">
							<div className="col-12">
								<div className="hrvmedit_content">
									...
									{/* VM creation */}
									<div id="hrvmedit_col_right">
										<div id="hrvmedit_col_right_row1">
											<div className="card">
												<div className="card-header">
										...
										<div id="hrvmedit_col_right_row2">
											<div className="text-center">
												<button className="btn btn-default btn-primary" onClick={ (e) => this.handleSubmit(e) }>Save</button>
											</div>
										...
										<div id="hrvmedit_col_right_row3">
											{/* 
											<br />
...
export default HRVacationMapEdit;